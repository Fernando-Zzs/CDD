...
public class Player extends MonoBehavior {
    ...
    private int id;
    private TransformAnimation passT;
    ...
    public Player(boolean isPlayer, int id, GameObject pass) {
        this.isPlayer = isPlayer;
        this.id = id;
        this.pass = (Transform) pass.getComponent(Transform.class);
        passT = (TransformAnimation) pass.getComponent(TransformAnimation.class);
    }
    ...
    public void Start() {
        ...
        if (Global.player_id == 1 && !blocked) {
            ...
            Collections.shuffle(GameSystem.getInstance().cards, rnd);
        }

        cardTable = (CardTable) getComponent(CardTable.class);
        ...
        for (int i = 0; i < 13; i++) {
            Card card = GameSystem.getInstance().deliverCard();

            card.setManager(this);
            if (isPlayer && Global.player_id == this.id) {
                card.addComponent(new BoxTouch());
                card.addComponent(new AutoTouch());
                card.addComponent(new TouchCardEvent());
                card.setSide(true);
            }
            ...
            if (card.getSuit() + card.getFigure() == 0) {
                GameSystem.getInstance().setFirstTurn(id);
            }
    ...
    public void updatePositions() {
        ...
        for (int i = 0; i < handCards.size(); i++) {
            ...
            card.position = transform.getPosition().add(
                    cardTable.calculatePosition(i, handCards.size()));
//            Log.d(TAG, "updatePositions: " + card.position.x + card.position.y + card.position.z);
            card.transformAnimation.beginMove(card.position, speed);
        }
    }
    ...
    public void outCardAnimation() {
        ...
        for (int i = 0; i < outCards.size(); i++) {
            ...
            card.position = Vector3.lerp(new Vector3(GameViewInfo.centerW, GameViewInfo.centerH - 100, 0), transform.getPosition(), (float) 0.3).add(
                    cardTable.calculateOutPosition(i, outCards.size(), GameSystem.getInstance().getTurnAmount()));
            card.transformAnimation.beginMove(card.position, speed);
    ...
    public void clearOutCards() {
        ...
        for (int i = 0; i < outCards.size(); i++) {
            Card card = outCards.get(i);
            card.transformAnimation.beginScale(Vector3.zero, speed);
        }
    ...
    public void showCardHandler() {
        ...
        outCardAnimation();
        //Judge
        GameSystem.getInstance().judgeCards(chosenCards);
        //Show
        ...
        GameSystem.getInstance().showCards(chosenCards);
    ...
    public void Update() {
        if (Global.proxy) { // 点击了托管
            enableShowCard = GameSystem.getInstance().judgeCards(chosenCards) && turn;
            if (GameSystem.getInstance().getTurnAmount() == 0 || GameSystem.getInstance().getLastPlayerID() == id)
                enablePass = false;
            else
                enablePass = turn;
            if (manager.turn&&manager.id==Global.player_id) {
                if (!thinking) {
                    lastCards = GameSystem.getInstance().getLastCards();
    ...
    private boolean chooseCards() {
        int minCardNum = 14;
        for (int i = 0; i < 4; i++) {
            minCardNum = Math.min(GameSystem.getInstance().getCardNums(i), minCardNum);
        }
        //新建cardPack
        ...
        ArrayList<Card> lastCards = GameSystem.getInstance().getLastCards();
        ...
        for (int i = 0; i < cardPack_5.size(); i++) {
            ...
            if (!GameSystem.getInstance().judgeCards(cards)) {
                for (int i1 = 0; i1 < cards.size(); i1++) {
        ...
        //顶大
        if (GameSystem.getInstance().getCardNums((manager.getId() + 1) % 4) <= threshold_MAX) {
            for (int i = 0; i < handCards.size(); i++) {
                for (int j = 0; j < 5; j++) {
        ...
        if (GameSystem.getInstance().getTurnAmount() == 0) {
            //阶砖3权重无敌
            Collections.sort(handCards);
            for (int i = 0; i < 5; i++) {
        ...
        //
        if (GameSystem.getInstance().getTurnAmount() == 0 || GameSystem.getInstance().getLastPlayerID() == manager.getId()) {
    ...
    private void sequentialCardMatch(ArrayList<ArrayList<Card>> cardPack_5, ArrayList<ArrayList<Card>> cardBucket, ArrayList<Card> cardsSample, int... i) {
        int cardType = GameSystem.getInstance().judgeCardType(cardsSample);
                    ...
                    for (int i3 = 0; i3 < cardBucket.get(i[2]).size(); i3++) {
                        ...
                        for (int i4 = 0; i4 < cardBucket.get(i[3]).size(); i4++) {
                            ...
                            for (int i5 = 0; i5 < cardBucket.get(i[4]).size(); i5++) {
                                ...
                                if (GameSystem.getInstance().judgeCardType(cards) == 4) {
                                    for (int k = 0; k < cards.size(); k++) {
